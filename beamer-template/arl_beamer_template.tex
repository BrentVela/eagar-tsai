  ...
  \begin{columns}[T,onlytextwidth]
    ...
    \begin{column}{0.40\textwidth}
      \begin{block}{Higher-Fidelity Thermal Reference}
        \begin{itemize}
          \item Composition-specific, temperature-dependent properties from TCHEA8 and Scheil calculations.
          \item Provides Marangoni-flow and keyholing modes, with evaporation, radiation, and convection.
          \item Matched composition, power, velocity, and beam dimensions enable direct comparison with Eagar--Tsai.
        \end{itemize}
      \end{block}
      ...
      \begin{block}{Outputs Used in This Work}
        \begin{itemize}
          \item 3D temperature field and melt-pool geometry
          \item $G$ and $R$ from downstream post-processing
        \end{itemize}
      \end{block}

      \vspace{-0.25em}
      \begin{alertblock}{Role in Bayesian Updating}
        \centering
        $\displaystyle
        T_{\mathrm{corrected}}
        = T_{\mathrm{ET}}
        + \widehat{\left(T_{\mathrm{TCAM}}-T_{\mathrm{ET}}\right)}$

        \vspace{0.35em}
        {\raggedright\fontsize{13.5}{16}\selectfont TCAM supplies sparse reference data used to learn and correct the ET discrepancy.\par}
      \end{alertblock}
    \end{column}
  \end{columns}
\end{frame}
